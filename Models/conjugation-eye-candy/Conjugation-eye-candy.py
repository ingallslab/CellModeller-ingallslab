...
def setup(sim):
    ...
    biophys = CLBacterium(sim, jitter_z=False, max_cells=30000,gamma=10.0, cgs_tol=1E-5,compNeighbours=True)
    ...
    sim.addCell(cellType=0, pos=(0,-5,0), dir=(1,0,0), length=delta,rad=0.4) #acceptor
    sim.addCell(cellType=0, pos=(0,10,0), dir=(0,1,0), length=delta,rad=0.4) #acceptor
    sim.addCell(cellType=1, pos=(5,0,0), dir=(1,0,0), length=delta,rad=0.4) #donor
    sim.addCell(cellType=1, pos=(0,5,0), dir=(1,0,0), length=delta,rad=0.4) #donor
    sim.addCell(cellType=1, pos=(0,-10,0), dir=(0,1,0), length=delta,rad=0.4) #donor
...
def init(cell):
    ...
    elif cell.cellType == 2: cell.color = (1.0,0.75,0.0)

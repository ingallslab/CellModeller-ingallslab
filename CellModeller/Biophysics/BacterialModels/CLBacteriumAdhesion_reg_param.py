...
class CLBacteriumAdhesion_reg_param:
    ...
    def __init__(self, simulator,
        ...
        self.window_average_size = window_average_size
    ...
    def addCell(self, cellState, pos=(0,0,0), dir=(1,0,0), rad=0.5, **kwargs):
        ...
        self.cell_force[i] = cellState.cellForce
    ...
    def init_kernels(self):
        ...
        kernel_AL_src = resource_string(__name__, 'AdhKernel.cl').decode()
        kernel_AL_src = kernel_AL_src % (adhLogicKernel) # comment this out to enable print statements in opencCL code
    ...
    def init_data(self):
        ...
        # cell and contact adhesion strentghs (AK) -AY
    ...
    def set_cells(self):
        ...
        self.cell_force_dev.set(self.cell_force)
    ...
    def updateCellState(self, state):
        ...
        window_size = self.window_average_size # time steps
        if state.cellAge > 0:
        ...
        # External forces
        for j in range(3):
            self.cell_force[i][j] = state.cellForce[j]
    ...
    def find_contacts(self, predict=True):
        ...
        self.program.find_plane_contacts_adh(self.queue,
                                         (self.n_cells,),
                                         None,
    ...
    def build_matrix(self):
        ...
        # Build B matrix using tangential directions to overlaps, 
        # and calculate overlap strengths -AY           
        self.program.build_tangent_matrix(self.queue,
                                             (self.n_cells, self.max_contacts),
                                             None,
                                             numpy.int32(self.max_contacts),    
                                             self.pred_cell_centers_dev.data,
                                             ...
                                             self.ct_pts_dev.data,
                                             self.ct_norms_dev.data,
                                             self.ct_tangs_fr_dev.data, # Updates this
                                             self.ct_tangs_to_dev.data, # Updates this
                                             self.ct_adh_str_dev.data, #Updates this
                                             self.ct_overlap_dev.data,
                                             self.ct_stiff_dev.data).wait()  

    def calculate_Ax(self, Ax, x, dt, alpha):
        # See Section 1.2.3 in CM maths. This calculates B_a*deltap_a - B_b*deltap_b -AY
        ...
        #Adhesion - calculating the tangential displacement to each contact and calculate adhesion energy
        self.program.calculate_Bx(self.queue,
                                  (self.n_cells, self.max_contacts),
                                  None,
                                  numpy.int32(self.max_contacts),
        ...
        '''
        Notes about math...
        # vaddkx(result, k, in1, in2) -> result[i] = in1[i] + k*in2[i]
        # in this case: A = A + (1/gamma)*BTBx
                        A = A + (1/gamma)*adhesion_energy
        '''
        self.vaddkx(Ax, self.reg_param, Ax, self.Mx_dev).wait()
        self.vaddkx(Ax, self.reg_param, Ax, self.adhE_dev).wait()
    ...
    def CGSSolve(self, dt, alpha, substep=False):
        ...
        # Optional computation of external forces                           
        if self.externalForces == True:  
